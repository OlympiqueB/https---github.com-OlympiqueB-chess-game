...
import { checkCheck } from "./core/CheckCheck.js";
...
function Chessboard() {
  ...
  function doesPossibleMoveExist(board, turn) {
    ...
    selectedPieces
      .flat(2)
      .forEach((t) =>
        hightlightPossibleMoves(t.row, t.col, board).forEach((attT) =>
          possibleMoves.add(attT)
        )
      );
  ...
  function handleClick(clickedRow, clickedCol) {
    ...
    // this is one long 'if' statement, but this handles all the game logic
    if (activePiece) {
      const possibleMoves = hightlightPossibleMoves(
        activeRow,
        activeCol,
        newBoard
      );
      ...
      if (
        enPasPawn.length !== 0 &&
        clickedRow != enPasPawn[0] &&
        clickedCol != enPasPawn[1]
      ) {
      ...
      if (
        newBoard[activeRow][activeCol].piece.type === "Pawn" &&
        clickedRow == (turn ? 7 : 0)
      ) {
      ...
      if (possibleMoves.includes(`${clickedRow}${clickedCol}r`)) {
        ...
        checkCheck(clickedRow, clickedCol, newBoard, setKingInCheck);
        setBoard(newBoard);
        newTurn(newBoard);
        return;
      }
      ...
      if (
        ...
        newBoard[clickedRow][clickedCol].piece.type === ""
      ) {
      ...
      if (possibleMoves.includes(`${clickedRow}${clickedCol}`)) {
        ...
        //stalemate
        if (
          !doesPossibleMoveExist(newBoard, turn) &&
          !checkCheck(clickedRow, clickedCol, newBoard, setKingInCheck)
        ) {
          setStalemate(true);
    ...
    deActivatePiece(newBoard);
    if (
      newBoard[clickedRow][clickedCol].piece.color ===
      (!turn ? "light" : "dark")
    ) {
  ...
  function activatePiece(r, c, board) {
    ...
    let activeTiles = hightlightPossibleMoves(r, c, board).map((t) =>
      t.split("")
    );
  ...
  function deActivatePiece(board) {
    ...
    let activeTiles = hightlightPossibleMoves(
      activePiece.split("")[0],
      activePiece.split("")[1],
      board
    ).map((t) => t.split(""));
  ...
  function handlePromotion(piece) {
    ...
    if (
      [...calcAttackedTiles(newBoard, turn)].filter(
        (t) => newBoard[t.split("")[0]][t.split("")[1]].piece.type === "King"
      ).length > 0
    ) {
      setKingInCheck(true);
    } else {
  ...
  function moveThePiece(r, c, board) {
    // this modifies the board input
  ...
  return (
    <div className="box">
      <div className="board">
        {board.map((row, rowIndex) =>
          row.map((tile, colIndex) => (
            <Tile
              ...
              kingCheck={
                kingInCheck &&
                tile.piece.type === "King" &&
                tile.piece.color === (!turn ? "light" : "dark")
              }
            />
          ))
        )}
        {showPromoteSelect && (
          <PawnPromotionSelect
            ...
            color={board[promRow][promCol].piece.color}
          />
        )}
      ...
      <div
        className="turn"
        style={{ backgroundColor: !turn ? "white" : "grey" }}
      >
